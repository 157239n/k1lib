...
def uploadF(): # installs in the local pypi server
    None | cmd(f". {uvAct}; cd {libFolder} && rm -rf {pypiFolder} && mkdir -p {pypiFolder} && rm -rf dist && {pythonExec} -m build && mv dist/* {pypiFolder}") | ignore()
    ls(pypiFolder) | apply(op().split("/")[-1]) | apply(lambda x: f"<a href='{x}'>{x}</a>") | join("") | aS(lambda x: f"<html>{x}</html>") | file(f"{pypiFolder}/index.html")
...
def oldDistributedInstall(): # installs the library in all nodes, should take 8-15s to complete
    base = libFolder; broken(); print("Starting distributedInstall()...")
...
def install():
    ...
    os.system(f". {uvAct}; {uvExec} pip uninstall k1lib")
    os.system(f". {uvAct}; {uvExec} pip install .")
...
def exportAll():
    ...
    fns = os.walk(libFolder) | cut(0, 2) | ungroup() | join("/").all() | grep(".py$") & grep(".ipynb$") | joinStreams() | apply(op().split(".") | head(-1).split() | join(".") + item()) | groupBy(0, True) | apply(item().all(), 1) | apply(list, 1) | filt("len(x) == 2", 1) | cut(0) | ~grep("checkpoint") | deref()
