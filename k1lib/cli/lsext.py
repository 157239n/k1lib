...
import k1lib as k1, os, json, k1lib, time, dill, threading, warnings
...
class PgManager: # singleton, to efficiently manages all connections from everywhere # PgManager
    ...
    def __call__(self, host, port, user, password, manage=False, backups=[]):    # PgManager
        if manage: raise Exception("Postgresql mode does not support remote management plane. Only works on sqlite") # PgManager
...
class MyManager:                                                                 # MyManager
    ...
    def __call__(self, host, port, user, password, manage=False, backups=[]):    # MyManager
...
class LiConn: # does not have restrictions such as changing database latency like postgresql/mysql, but still keep the same structure to make everything consistent # LiConn
    def __init__(self, conn, fn, backups:"list[str]", wal:bool):                 # LiConn
        self.conn = conn; self.fn = fn; self.backups = backups; self.wal = wal; self._lastrowid = None; self.connected = False; self.lock = threading.Lock(); self.connect() # LiConn
        if backups != []:                                                        # LiConn
...
class LiConns:                                                                   # LiConns
    def __init__(self, fn:str, backups:"list[str]", wal:bool): self.conn = LiConn(None, fn, backups, wal); self.fn = fn # LiConns
...
class LiManager:                                                                 # LiManager
    ...
    def __call__(self, host:str, port, user, password, manage=False, backups=[], wal=True): # LiManager
        if host == ":memory:": return LiConns(host, backups, wal) # no caching, cause each instance is different! # LiManager
        host = os.path.expanduser(host)                                          # LiManager
        if host not in self.connss: self.connss[host] = LiConns(host, backups, wal) # LiManager
...
:param kwargs: optional arguments for db connector constructors. See docs above for options""" # sql
        ...
        self.conn = self.manager(self.host, self.port, self.user, self.password, manage, backups, **kwargs)[None] # sql
...
<div style="display: flex; flex-direction: row; align-items: center"><h1 style="margin-right: 24px">Database '{fn}' - table '{tblName}'</h1><button onclick="window.location = '/k1/sqlite/{sand}';" style="padding: 8px">Back</button></div><pre>{info}</pre>
...
<textarea id="{pre}_query" style="width: 350px; height: 200px"></textarea><div id="{pre}_duration"></div><pre id="{pre}_result"></pre>
<script>{pre}_queryBtn.onclick = async () => {{ try {{
    {pre}_duration.innerHTML = ""; {pre}_result.innerHTML = "Executing...";
