...
:param url: (optional) url of the package's website, so that they know where to get official docs""" # dep
    ...
    def _k_bootstrap(self):                                                      # dep
        try: self._k_mod = importlib.import_module(self._k_s); return self._k_mod # dep
        except:                                                                  # dep
            after = f"More information is available on {self.url}. " if self.url else f"" # dep
...
:param timeout: timeout in seconds"""                                            # cache
    ...
    def __call__(self, func):                                                    # cache
        ...
        def wrapper(*args):                                                      # cache
            ...
            key = ((tuple(x) if isinstance(x, list) else x) for x in args) # can accept lists of hashable items! # cache
