...
import logging, warnings, os, time, re, gc, json, k1lib, importlib, urllib.parse, math, base64, dill, inspect, asyncio, signal, contextlib, sys
...
__all__ = ["_docsUrl", "mplLock", "dep", "depCli", "deprecated", "argNames", "isNumeric", "hashPw", "verifyPw", "leakedPw",
           "patch", "shutdownEvent", "managePlanes", "cache", "cron", "preload", "wrapMod", "wraps", "squeeze", "raiseEx",
           ...
           "encode", "decode", "hash", "resolve", "resolveAsyncGen", "config", "modbusCrc", "parseTimeStr", "extractLinks"]
...
dep.cv2 = dep("cv2", "opencv-python", "https://opencv.org/"); dep.bloom = dep("pybloom", url="https://github.com/jaybaird/python-bloomfilter") # dep
dep.bcrypt = dep("bcrypt", url="https://github.com/pyca/bcrypt")                 # dep
...
def deprecated(reason:str):                                                      # deprecated
    """Decorator to mark a function as being deprecated.
Example::

    @k1.deprecated("some reason")
    def someFunc():
        pass
"""                                                                              # deprecated
    def inner(f): return f # will fill this out with may be another management plane, at /k1/deprecated or something like that? But for now, just want something to notify to others # deprecated
    return inner                                                                 # deprecated
def argNames(f):                                                                 # argNames
    """Grabs argument names for a function.
Example::

    def someFunc(x, y, z):
        return x + y - z
    k1.argNames(someFunc) # returns ["x", "y", "z"]
"""                                                                              # argNames
    import inspect; return inspect.getfullargspec(f).args                        # argNames
...
basePath = os.path.dirname(inspect.getabsfile(k1lib)) + os.sep                   # isNumeric
def hashPw(password:"str|bytes", rounds:int=12) -> str:                          # hashPw
    """Hashes password to safely store it in a database.
Example::

    k1.hashPw("password") # might return '$2b$12$N2.O9cqMjMG9caKcyHwbTeHbC6Q8Sf5xCZz9PrfWBPvNmRuEUI.h.'

These uses bcrypt behind the scenes to hash, which is secure, but also slow. The
higher the rounds, the more secure and the slower it is. Speed table:

- rounds=12: 336 ms ± 24.2 ms
- rounds=11: 179 ms ± 16.2 ms
- rounds=10: 96.1 ms ± 9.24 ms
- rounds=9: 44.9 ms ± 2.78 ms
- rounds=8: 21.5 ms ± 618 μs
- rounds=7: 11.4 ms ± 997 μs
- rounds=6: 5.04 ms ± 98.8 μs
- rounds=5: 2.75 ms ± 114 μs
"""                                                                              # hashPw
    if isinstance(password, str): password = password.encode()                   # hashPw
    import bcrypt; salt = bcrypt.gensalt(rounds=rounds); return bcrypt.hashpw(password, salt).decode() # hashPw
def verifyPw(password:"str|bytes", hashedPw:str) -> bool:                        # verifyPw
    """Verify password with hashed password to see if it's correct or not.
Example::

    hashedPw = k1.hashPw("password")
    k1.verifyPw("password", hashedPw) # returns True

Speed figures similar to :meth:`k1.hashPw`, and depends on the number of rounds""" # verifyPw
    if isinstance(password, str): password = password.encode()                   # verifyPw
    import bcrypt; return bcrypt.checkpw(password, hashedPw.encode())            # verifyPw
_pw100k_bloom = [None]                                                           # verifyPw
def leakedPw(password:"str|bytes") -> bool:                                      # leakedPw
    """Checks whether the supplied password has been leaked.
Internally uses the top 100k common passwords, encoded in a bloom filter
with false positive rate of 1/100000, or 0.001%. Example::

    k1.leakedPw("password") # returns True
    k1.leakedPw("12345678") # returns True
    k1.leakedPw("correcthorsebatterystaple") # returns False, but still don't use this password

Takes: 7.7 μs ± 1.05 μs

Requires python module `pybloom`. Data comes from the site HaveIBeenPwned, and the
top 100k common passwords have occurances more than 11158. 'correcthorsebatterystaple'
has occurances of like ~100, so it flies under the radar."""                     # leakedPw
    import pickle, hashlib                                                       # leakedPw
    if _pw100k_bloom[0] is None:                                                 # leakedPw
        with open(f"{basePath}pw_top100k_bloom.bin", "rb") as f: _pw100k_bloom[0] = pickle.loads(f.read()) # leakedPw
    return hashlib.sha1(password.encode()).digest().hex().upper() in _pw100k_bloom[0] # leakedPw
...
"""                                                                              # patch
    ...
    return inner                                                                 # patch
shutdownEvent = threading.Event(); import atexit                                 # patch
def handle_sigint(): print("\nSIGINT received. Stopping crons..."); shutdownEvent.set(); time.sleep(2) # real shutdown after leaving 2 second window to shut things down gracefully # handle_sigint
atexit.register(handle_sigint)                                                   # handle_sigint
class managePlanes:                                                              # managePlanes
    planes = []                                                                  # managePlanes
    attached = False                                                             # managePlanes
    @staticmethod                                                                # managePlanes
    def flask(app, **kwargs):                                                    # managePlanes
        """Attaches a general management plane to a flask app.
Example::

    app = flask.Flask(__name__)
    k1.managePlanes.flask(app)
    app.run(host="0.0.0.0", port=80)

Then, you can access the route "/k1" to see an overview of all available planes

Update: now, attaching any specific management planes will add this automatically,
so manual interaction with this is no longer necessary

:param app: flask app object
:param kwargs: extra random kwargs that you want to add to ``app.route()`` function""" # managePlanes
        if managePlanes.attached: return                                         # managePlanes
        viz = k1lib.viz; managePlanes.attached = True                            # managePlanes
        @app.route("/k1", **kwargs)                                              # managePlanes
        def _k1():                                                               # managePlanes
            pre = k1lib.cli.init._jsDAuto(); ui1 = managePlanes.planes | viz.Table(["name", "path", "desc"], onclickFName=f"{pre}_select") # managePlanes
            return f"""<h1>Management planes</h1><div style="overflow-x: auto">{ui1}</div><script>function {pre}_select(row, i, e) {{ window.location = row[1]; }}</script>""" # managePlanes
    @staticmethod                                                                # managePlanes
    def append(name:str, path:str, desc:str): managePlanes.planes.append([name, path, desc]) # managePlanes
...
NoneType = type(None); _k1_cache_autoInc = [1]; _k1_cacheD = {} # Dict[name -> {docs, func, clearF, hits, misses}] # hashableAKw
class cache:                                                                     # cache
    def __init__(self, maxsize=128, timeout=60, name=None, docs=None, fn:str=None, pickle:"bool|tuple"=False): # cache
        """Like functools.lru_cache, but with extra bells and whistles.
Example::
...
You can also save cached results to disk::

    @k1.cache(fn="abc")
    def slow_function(x): return x*2

This has several things to keep in mind as well as caveats:
- keys will be converted into strings using str() before saving to sqlite
- values returned by the wrapped function has to be either (int, float, str, bytes, None), if .pickle is not specified
- maxsize only works for in-memory cache. When it is freed in memory, it will also be cleared on disk. Cache on disk can grow indefinitely

If .pickle is specified, then there are 2 different options:
- pickle == True: this will use the package "dill" to serialize and deserialize all values returned by the function
- pickle is a tuple with 2 functions, 1 to serialize and the other to deserialize: will use the specified functions instead

You can also check whether the function already has a key or not by doing this::

    @k1.cache()
    def func(x): return x+2
    f(3)
    func.cacheObj.hasKey(3) # returns True
    func.cacheObj.hasKey(4) # returns False

:param maxsize: max size of the cache
:param timeout: timeout in seconds, put 0 or None to disable timeout
:param name: short name of the cache, to be displayed in management plane
:param docs: docs of the cache, also displayed in management plane
:param fn: sqlite file name. If specified, will save cached results on disk in sqlite database""" # cache
        ...
        self.maxsize = maxsize; self.timeout = timeout; self.cache = collections.OrderedDict(); self.fn = fn; self.tbl = None; self._sqliteReady = False # cache
        if pickle == False: pickle = (lambda x:x, lambda x:x)                    # cache
        elif pickle == True: pickle = (dill.dumps, dill.loads)                   # cache
        elif isinstance(pickle, tuple): assert len(pickle) == 2, f"pickle is a tuple of length different than 2 ({len(pickle)})" # cache
        else: assert False, f"pickle is of unrecognized type {type(pickle)}"     # cache
    ...
    def hasKey(self, *args, **kwargs):                                           # cache
        ...
        if key in self.cache: return True                                        # cache
        return self.fn and self.tbl.lookup(key=str(key))                         # cache
    def _ensureRow(self, row, key): return row if row else self.tbl.lookup(key=str(key)) # cache
    def _updateRow(self, row, key, value, pickledValue=None):                    # cache
        row = self._ensureRow(row, key); now = int(time.time()); value = self.pickle[0](value) if pickledValue is None else pickledValue # cache
        if row: row.value = value; row.time = int(now)                           # cache
        else: row = self.tbl.insert(key=str(key), value=value, time=int(now))    # cache
        return row                                                               # cache
    def __call__(self, func):                                                    # cache
        if self.name in _k1_cacheD: warnings.warn(f"Name '{self.name}' has appeared before. Reusing the same name means the older process will be lost to the cache management plane") # cache
        ...
        def wrapper(*args, _k1_cache_refresh=False, **kwargs): # _k1_cache_refresh: hidden feature, if True, returns the cached item, but then immediately fetches the new version. Not entirely sure where this is used in # cache
            if self.maxsize == 0: return func(*args, **kwargs)                   # cache
            key = hashableAKw(args, kwargs); now = time.time(); result = None; fn = self.fn; row = None; self._inMem = inMem = key in self.cache # cache
            if fn: self._setupSqlite()                                           # cache
            if inMem: result, timestamp = self.cache[key]                        # cache
            elif fn:                                                             # cache
                row = self.tbl.lookup(key=str(key))                              # cache
                if row: result = self.pickle[1](row.value); timestamp = row.time # cache
            if result:                                                           # cache
                if fn and not inMem: self.cache[key] = (result, timestamp)       # cache
                if _k1_cache_refresh:                                            # cache
                    result = func(*args, **kwargs); self.cache[key] = (result, now) # cache
                    if fn: self._updateRow(row, key, result)                     # cache
                    return result                                                # cache
                if self.timeout:                                                 # cache
                    if now - timestamp < self.timeout:                           # cache
                        if inMem: self.cache.move_to_end(key);                   # cache
                        meta["hits"] += 1; self._status = "hit"; return result   # cache
                    else:                                                        # cache
                        if inMem: del self.cache[key]                            # cache
                        if fn:                                                   # cache
                            row = self._ensureRow(row, key)                      # cache
                            if row: del self.tbl[row.id]                         # cache
                        self._status = "timeout"; meta["timeouts"] += 1          # cache
                else: self._status = "hit"; return result                        # cache
            else: meta["misses"] += 1; self._status = "miss"                     # cache
            result = func(*args, **kwargs)                                       # cache
            if fn:                                                               # cache
                pickedResult = self.pickle[0](result)                            # cache
                if not isinstance(pickedResult, (int, float, bytes, str, NoneType)): raise Exception(f"Cache '{self.name}' wraps a function that returns value of type {type(pickledResult)}, which can't be stored in sqlite. Please modify the wrapped function so that it returns one of (int, float, str, bytes, None), or .pickle has to be set to True or tuple of ser/deser functions") # cache
            if len(self.cache) >= self.maxsize:                                  # cache
                k,v = self.cache.popitem(last=False)                             # cache
                if fn: del self.tbl[self._ensureRow(None, k).id] # if in mem cache, then definitely in table! # cache
            if fn: self._updateRow(row, key, result, pickedResult)               # cache
            self.cache[key] = (result, now); meta["len"] = len(self.cache); return result # cache
        wrapper.cacheObj = self; return wrapper                                  # cache
    @staticmethod                                                                # cache
    def allData():                                                               # cache
...
:param kwargs: extra random kwargs that you want to add to ``app.route()`` function""" # cache
        k1 = k1lib; cli = k1.cli; viz = k1.viz; managePlanes.append("cache", "/k1/cache", "Cache management"); managePlanes.flask(app, **kwargs) # cache
...
<div style="display: flex; flex-direction: row; align-items: center"><h1>Caches</h1><button style="margin-left: 24px; padding: 8px" onclick="window.location='/k1';">Back</button></div>
<button onclick="clearAllCache()">Clear all caches</button><div style="overflow-x: auto">{ui3}</div><script>async function clearCache(x) {{ await fetch(`/k1/cache/api/${{x}}/clear`); alert("Cleared"); }}; async function clearAllCache() {{ await fetch(`/k1/cache/api/clearAll`); alert("Cleared all"); }}</script>""" # cache
...
class cron:                                                                      # cron
    ...
    def __init__(self, f=None, delay=None, kw=None, name=None, docs=None, daemon=False, delayedStart=0, ctx=None): # cron
        """Sets up a cron job in another thread, running the decorated
function in 2 modes:

Mode 1: whenever ``f`` goes from False to True (aka positive edge). Does nothing on the
negative edge. It's activated when ``delay`` is not specified. Example::
...
The precise day/hour/etc is with respect to the timezone at :data:`~k1lib.settings`.timezone.
A list of timezones is available at ``import zoneinfo; zoneinfo.available_timezones()``

.. warning::
    TL;DR: don't use the ``second`` argument.

    The function will be triggered on the positive edge. However, the current time
    will be evaluated only every 0.5 seconds, and so if python's does not context-switch
    back to the time evaluation function, then there can be an additional negative edge
    before the system has time to respond, which will not trigger the main function!
...
:param ctx: if specified, wrap the cron background loop in the specified context. May be you want to
    activate cron of 1 worker process only, so you can put k1.SharedLock() here""" # cron
        ...
        self.f = f; self.delay = delay; self.kw = kw; self.name = name; self.docs = docs; self.daemon = daemon; self.delayedStart = delayedStart; self.ctx = ctx or contextlib.nullcontext() # cron
    @staticmethod                                                                # cron
    def allData():                                                               # cron
        """Grabs ``Dict[name -> {docs, delay, logs}]`` for all delay crons"""    # cron
        return _k1_cron_delay_data                                               # cron
    def __call__(self, func):                                                    # cron
        f = self.f; delay = self.delay; kw = self.kw; name = self.name; docs = self.docs; kw = kw or {}; isAsync = inspect.iscoroutinefunction(func); _startSyncScanCron(); func2 = k1lib.logErr()(func) # cron
        ...
        if delay:                                                                # cron
                ...
                def startLoop():                                                 # cron
                    ...
                    with self.ctx:                                               # cron
                        while True:                                              # cron
                            ...
                            try:                                                 # cron
                                _k1_cron_delay_data[name]["lastRun"] = [beginTime, "now"]; res = func2(**kw) # cron
                ...
                async def startALoop():                                          # cron
                    ...
                    with self.ctx:                                               # cron
                        while True:                                              # cron
                            ...
                            beginTime = time.time(); beginTTime = time.thread_time() # cron
                            try:                                                 # cron
                                _k1_cron_delay_data[name]["lastRun"] = [beginTime, "now"]; res = await func2(**kw) # cron
                                if res == cron.stop: break                       # cron
                            except Exception as e: print(f"{type(e)}\ue004{e}"); print(traceback.format_exc()); _k1_cron_delay_data[name]["logs"].append({"beginTime": beginTime, "duration": time.time() - beginTime, "success": False, "exc": f"{type(e)}\ue004{e}", "tb": traceback.format_exc()}); k1lib.logErr.rawErr(func, e) # cron
            ...
            try: import zoneinfo; tz = zoneinfo.ZoneInfo(k1lib.settings.timezone) if k1lib.settings.timezone else None # cron
            except: tz = None                                                    # cron
            args = list(inspect.signature(f).parameters.keys())                  # cron
            s = {"year", "month", "day", "weekday", "hour", "minute", "second"}  # cron
            sleepDuration = 0.5 if "second" in args else (15 if "minute" in args else 50) # progressively longer sleep times to reduce cpu load # cron
            ...
            if not isAsync:                                                      # cron
                def startLoop():                                                 # cron
                    ...
                    with self.ctx:                                               # cron
                        while True:                                              # cron
                            if not threadRef[2]: break                           # cron
                            beginTTime = time.thread_time(); a = datetime.datetime.now(tz) # cron
                            ...
                            if not last and this:                                # cron
                                try:                                             # cron
                                    if func2(**kw) == cron.stop: break           # cron
                ...
                async def startALoop():                                          # cron
                    ...
                    with self.ctx:                                               # cron
                        while True:                                              # cron
                            ...
                            if not last and this:                                # cron
                                try:                                             # cron
                                    if await func2(**kw) == cron.stop: break     # cron
                                except Exception as e: print(f"{type(e)}\ue004{e}"); print(traceback.format_exc()); k1lib.logErr.rawErr(func, e) # cron
...
:param kwargs: extra random kwargs that you want to add to ``app.route()`` function""" # cron
        k1 = k1lib; cli = k1.cli; viz = k1.viz; managePlanes.append("cron", "/k1/cron", "Cron jobs"); managePlanes.flask(app, **kwargs) # cron
        def k1_thread():                                                         # cron
        ...
        def k1_cron_index():                                                     # cron
            ui1 = k1.cron.allData().items() | ~cli.apply(lambda k,v: [k, v["delay"], v["delayedStart"], v["lastRun"][1], v["func"].__name__, inspect.getfile(v["func"]), inspect.getsourcelines(v["func"])[1], v["docs"]]) | cli.deref() | (cli.toJsFunc("term") | cli.grep("${term}") | viz.Table(["name", "delay", "delayedStart", "lastRun", "func's name", "func's file name", "line number", "docs"], height=600, sortF=True)) | cli.op().interface() | cli.toHtml() # cron
            ...
            return f"""<div style="display: flex; flex-direction: row; align-items: center"><h1>Crons</h1><button style="margin-left: 24px; padding: 8px" onclick="window.location='/k1';">Back</button></div>
<h2>Overview of all cron jobs</h2><div style="overflow-x: auto">{ui1}</div><h2>Overview of all threads, including cron jobs</h2><div style="overflow-x: auto">{k1_thread()}</div><h2>Detailed cron job's object</h2><div style="overflow-x: auto">{ui2}</div>""" # cron
...
hash(34)                                                                         # hash
_resolve_loop = [None]                                                           # hash
def _resolve_start():                                                            # _resolve_start
    if _resolve_loop[0]: return _resolve_loop[0]                                 # _resolve_start
    _resolve_loop[0] = asyncio.new_event_loop()                                  # _resolve_start
    def _resolve_worker(): asyncio.set_event_loop(_resolve_loop[0]); _resolve_loop[0].run_forever() # _resolve_start
    threading.Thread(target=_resolve_worker, daemon=True).start(); return _resolve_loop[0] # _resolve_start
def resolve(co, eventLoop=None):                                                 # resolve
...
in a single async function, then .resolve() the whole thing

See also: :meth:`k1lib.resolveAsyncGen`"""                                       # resolve
    return asyncio.run_coroutine_threadsafe(co, eventLoop or _resolve_start()).result() # resolve
import functools, asyncio                                                        # resolve
class AsyncGenReturn(Exception):                                                 # AsyncGenReturn
    def __init__(self, value): super().__init__(); self.value = value            # AsyncGenReturn
def resolveAsyncGen(eventLoop=None):                                             # resolveAsyncGen
    """Converts async generator into sync generator.
Example::
...
A more complicated example::
    ...
    async def aF(x, y):
        yield x + y; yield x * y
        raise k1.resolveAsyncGen.Return(x - y)
...
:param eventLoop: Custom event loop. If not specified, use the same event loop as :meth:`k1lib.resolve`""" # resolveAsyncGen
    eventLoop = eventLoop or _resolve_start()                                    # resolveAsyncGen
    def inner(async_gen_func):                                                   # resolveAsyncGen
        @functools.wraps(async_gen_func)                                         # resolveAsyncGen
        def wrapper(*args, **kwargs):                                            # resolveAsyncGen
            agen = async_gen_func(*args, **kwargs)                               # resolveAsyncGen
            try:                                                                 # resolveAsyncGen
                while True:                                                      # resolveAsyncGen
                    try: yield resolve(agen.__anext__(), eventLoop)              # resolveAsyncGen
                    except AsyncGenReturn as e: return e.value                   # resolveAsyncGen
                    except StopAsyncIteration: return                            # resolveAsyncGen
            finally:                                                             # resolveAsyncGen
                try: resolve(agen.aclose(), eventLoop)                           # resolveAsyncGen
                except Exception: pass                                           # resolveAsyncGen
        return wrapper                                                           # resolveAsyncGen
    return inner                                                                 # resolveAsyncGen
resolveAsyncGen.Return = AsyncGenReturn                                          # resolveAsyncGen
