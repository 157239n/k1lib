...
_serve_d = k1lib.serve.analyze(endpoint); _serve_args = _serve_d["args"]; _serve_annos = _serve_d["annos"]; _serve_privates = _serve_d["privates"]; _serve_defaults = _serve_d["defaults"]
...
def _serve_main():
    try:
        ...
        for arg in [arg for arg in _serve_args if _serve_annos[arg] == "apiKey"]: # if has serve.apiKey(), then check if the key matches. If not, then raise exception
            if js[arg] != _serve_privates[arg]: raise Exception(f"Api key '{arg}' wrong or is not specified") # _serve_defaults[arg][0] part is not airtight, as there're exceptions like dropdown, but good enough for my purpose
        return {"data": k1lib.serve.pyToWeb(endpoint(*[k1lib.serve.webToPy(js[arg] if arg in js else _serve_defaults[arg][0], _serve_annos[arg]) for arg in _serve_args]), _serve_annos["return"]), "success": True}
